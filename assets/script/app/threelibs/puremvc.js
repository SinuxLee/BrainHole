 ...
 */
  View.prototype.notifyObservers = function (notification) {
    ...
    if (this.observerMap[notification.getName()] != null) {
      const observers_ref = this.observerMap[notification.getName()]; const observers = []; let observer
  ...
  const OopHelp =
{
  ...
  extend: function (child, parent) {
    if (typeof child !== 'function') { throw new TypeError('#extend- child should be Function') }

    if (typeof parent !== 'function') { throw new TypeError('#extend- parent should be Function') }

    if (parent === child) { return }

    const Transitive = new Function()
 ...
 */
  function declare (qualifiedName, object, scope) {
    const nodes = qualifiedName.split('.')
    let node = scope || OopHelp.global
    let lastNode
    let newNode
    let nodeName
    ...
    for (let i = 0, n = nodes.length; i < n; i++) {
      ...
      node = (node[nodeName] == null ? node[nodeName] = {} : node[nodeName])
    }
 ...
 */
  function define (classInfo, traits, staticTraits) {
    ...
    const classParent = classInfo.parent
    const doExtend = typeof classParent === 'function'
    let classConstructor
    const classScope = classInfo.scope || null
    let prototype
...
puremvc.Facade = puremvc.Facade.getInstance('game')
