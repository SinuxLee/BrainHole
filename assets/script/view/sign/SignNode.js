...
const SignNodeMediator = require('SignNodeMediator')
...
cc.Class({
  ...
  onLoad () {
    ...
    const closebtn = topbg.getChildByName('closebtn')
    closebtn.on('click', () => {
    ...
    const tasknode = this.node.getChildByName('tasknode')
    // 1.签到
    ...
    this.qd_getbtn.on('click', () => {
      ...
      const srcPos = cc.v2(cc.winSize.width / 2, cc.winSize.height / 2)
      const targetPos = this.keyicon.convertToWorldSpaceAR(cc.v2(0, 0))
  ...
  setSignState () {
    const signDay = cc.sys.localStorage.getItem('signDay') || '0,0'
    const signDayNum = signDay.split(',')
  ...
  setTgState () {
    const tgLevels = [10, 50, 100, 144, 160]
    const taskPointNum = cc.sys.localStorage.getItem('taskPointNum') || 1 // 已通关领取奖励的关卡数
    this.taskPointNum = 200
    for (let i = 0; i < tgLevels.length; ++i) {
  ...
  setVedioState () {
    const tastVedio = cc.sys.localStorage.getItem('tastVedio') || '0,0'
    const tastVedioNum = tastVedio.split(',')
    ...
    const time = Number(tastVedioNum[1] - today)
    if (time < 0) {
