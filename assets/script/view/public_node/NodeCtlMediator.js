cc.Class({
  ...
  showToast (data) {
    const prefab = cc.loader.getRes('prefab/ToastNode')
  ...
  getNodeAssets (name) {
    let assets = appConfig.depends[name]
    const assetsFunction = appConfig.depends[name + 'GetRes']
    if (!assets) {
      assets = []
    }
    if (assetsFunction) {
    ...
    for (let i = 0; i < assets.length; i++) {
      const arr = assets[i].split('_')
      const atype = arr[0]
      const aname = arr[1]
      if (atype == 'prefab') {
        const nodeAsserts = this.getNodeAssets(aname)
        if (nodeAsserts) {
  ...
  showPop (name, loading, initData) {
    ...
    if (this.popNodePool[name]) {
      ...
      const mediator = this.facade.retrieveMediator(name)
      ...
      if (preloadTool.getPrefab(name)) {
        ...
        data.groupName = name
        const assets = this.getNodeAssets(name)
  ...
  preloadComplete (name, loadType, initData, actionType) {
    const prefab = cc.loader.getRes('prefab/' + name)
    const node = cc.instantiate(prefab)
    switch (loadType) {
  ...
  currentNodeHasAssert (name) {
    let isHad = false
    for (let i = 0; i < this.nodeAssertsObj[this.currentNodeName].length; i++) {
      const arr = this.nodeAssertsObj[this.currentNodeName][i].split('prefab_')
  ...
  destroyAllPop () {
    for (const key in this.popNodePool) {
